...
class LitModel(pl.LightningModule):
    def __init__(self,args=None):
        ...
        self.proj = torch.nn.Conv2d(768, 1024, kernel_size=1, bias=False)
        ##
    ...
    def training_step(self, batchs):
        ...
        teac_head = self.teacher.head
        tensor_spatial = F.interpolate(mid_spike.mean(0), size=(7, 7), mode='bilinear', align_corners=False)
        adjusted_tensor = self.proj(tensor_spatial)
        adjusted_tensor = adjusted_tensor.permute(0, 3, 2, 1)
        # with torch.no_grad():
        mix_res = teac_head(adjusted_tensor)
        ...
        roi_images, roi_labels = sample_roi_from_prob_diversity(batch, layer3_out_fire_rate, gt, 
                                                                num_rois=6, roi_size=(self.crop_size, self.crop_size))
        ...
        #from utils.utils import mmd_loss,getForwardCAM,compute_kl_divergence,freeze_model_parameters,soft_loss_smooth,logits_external_loss,logits_internal_loss,soft_loss_hard 
        af_kd = soft_loss_hard(student_logits=out,teacher_logits=mix_res,temperature=self.Tau)

        
        loss = self.criterion(out, gt)  + sample_loss  + roi_kl_distill_loss(out, out_t_sample,num_rois=6,T=self.Tau)+ self.hyper_cam*l3 + af_kd
    ...
    def configure_optimizers(self):
        ...
        if self.opter == 'Adam':
            optimizer = torch.optim.Adam(
                nn.ModuleList([self.feature_extractor,self.proj]).parameters(), lr=self.learning_rate, 
                # self.feature_extractor.parameters(), lr=self.learning_rate, 
                #  lr=1e-4,
                weight_decay=1e-4
            )
...
def parse_args():
    ...
    # nohup python pl_train_kd_our_sample_snn_w_enof.py --config config/cubs200/plt_our_kdtrain_snn_cub_enof.yml --print-yaml &
...
def main():
    ...
    if args.is_distributed:
        trainer = pl.Trainer(logger=logger, max_epochs=args.max_epochs, accelerator="gpu",callbacks=[checkpoint_callback],strategy="ddp_find_unused_parameters_true",precision=args.mixed)
    else:
